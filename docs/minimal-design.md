...
The extension uses the stable `vscode.lm.selectChatModels` surface exposed by VS Code. Providers supplied by other extensions declare `contributes.languageModelChatProviders` and register their vendor with VS Code; Code Subtitle consumes their returned `LanguageModelChat` objects and does not register a provider or call a custom API. The returned `vendor` is a provider identifier, and `id` is unique within that provider. [Language Model Chat Provider API](https://code.visualstudio.com/api/extension-guides/ai/language-model-chat-provider)

An explicit `codeSubtitle.model` value accepts `vendor:id`. A bare ID remains a Copilot ID for compatibility with the earlier preview. The adapter passes both fields to `selectChatModels`, then verifies the returned model has the exact vendor and ID. For `auto`, it first queries `{ vendor: 'copilot' }`; when that list is empty, it queries without a selector and presents every available provider. Picker entries display and persist the vendor-qualified key (`vendor:id`) so equal IDs from different providers cannot collide. The global-state key is `codeSubtitle.autoModelKey`; the previous `codeSubtitle.autoModelId` value is read as a legacy Copilot ID. A model already selected for the current session remains fixed; catalog changes, configuration changes, and unavailable-model failures invalidate it rather than silently switching vendors mid-request.

Do not make a particular model name or speed a permanent assumption. Reuse the selected model, and invalidate the in-memory selection when `lm.onDidChangeChatModels` fires or an unavailable-model failure occurs; the stored key is kept and re-validated against the available models on the next resolve. If the stored model is no longer available, show the picker again and overwrite the stored key. [Model selection API](https://code.visualstudio.com/api/references/vscode-api#lm)

Even a route that does not require an API key may require provider-specific sign-in, model-use permission, consent, configuration, and available quota. Start model retrieval from a user operation. On first use, briefly explain what will be submitted and leave consent or configuration to the selected provider's VS Code flow. Do not add a duplicate confirmation of your own; if access is refused, do not submit.
