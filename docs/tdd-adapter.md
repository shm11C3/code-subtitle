...
The gateway always queries the `copilot` vendor. A configured model setting must match an available model ID exactly. With `auto`, the gateway presents the available models in the order returned by VS Code and remembers the user's one Quick Pick choice through the injected `ModelChoiceStore` (the extension backs it with `globalState`); it does not rank or label models with invented speed claims. Model catalog and configuration changes invalidate the in-memory model only; the stored ID is re-validated on the next resolve and replaced through the picker when it is no longer available.
