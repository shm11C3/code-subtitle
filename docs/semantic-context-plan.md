...
Collect after the cache lookup and before token fitting. The cache identity is the bounded, pre-enrichment input plus the policy version and model identity, so a hit performs no provider query and no token count. This is safe because enrichment and fitting are deterministic for a given model and workspace state, and the extension invalidates the workspace cache on any source change while semantic context is enabled and clears it when `codeSubtitle.semanticContext`, the model, or the language settings change. On a miss, fit the complete prompt; discard optional semantic evidence before reducing adjacent context, and never truncate the selection. Do not cache provider responses separately.
